...
export async function getProjectAnalytics(): Promise<ProjectAnalytics> {
  try {
    ...
    allProjects.forEach((project) => {
      ...
      project.techStack.frontend.forEach((tech: string) => {
        techUsageStats.frontend[tech] =
          (techUsageStats.frontend[tech] || 0) + 1;
      });

      // Count backend technologies
      project.techStack.backend.forEach((tech: string) => {
        techUsageStats.backend[tech] = (techUsageStats.backend[tech] || 0) + 1;
      });

      // Count database technologies
      project.techStack.database.forEach((tech: string) => {
        techUsageStats.database[tech] =
          (techUsageStats.database[tech] || 0) + 1;
      });

      // Count infrastructure technologies
      project.techStack.infrastructure.forEach((tech: string) => {
        techUsageStats.infrastructure[tech] =
          (techUsageStats.infrastructure[tech] || 0) + 1;
      });
    });
    ...
    console.error(
      "Error fetching project analytics, using fallback data:",
      error
    );
